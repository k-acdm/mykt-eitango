...
A: (ax + b)(cx + d) — 基本展開（a, c は ±1〜±2、4 級と区別するため a*c ≠ 1 を意識）
B: (ax + b)(cx + d) — 一般係数（a, c は ±1〜±5）
C: 3項 × 2項 — (ax² + bx + c)(dx + e) など

すべて単一変数 x、整数係数のみ（分数係数は §6.4.0 既約性必須だが Phase 3 で扱う）。
TODO_PHASE3: 多変数（x, y）の展開、分数係数の展開は Phase 3 の Band D 以降で導入。
"""
...
def _gen_two_by_two(rng, coef_max, const_max):
    ...
    # 自明な (1)(1) のような冗長を避ける
    # a == c == 1 で b == d なら (x+a)^2 になり 4 級と被るが採用は許容
    # 結果係数：(ac, ad+bc, bd)
    c2 = a * c
    c1 = a * d + b * c
    c0 = b * d
...
def generate_problem(band: str, rng: random.Random) -> Dict[str, Any]:
    ...
    for _ in range(300):
        if kind == "two_by_two_simple" or kind == "two_by_two_general":
            built = _gen_two_by_two(rng, cfg["coef_max"], cfg["const_max"])
