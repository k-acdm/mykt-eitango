...
1. 全角英数 → 半角（case 保持）
2. 類似句読点を半角統一（`、→,` `．→.` 等）。但し「。」は保持（日本文末記号）
3. **すべての空白文字を削除**（`/[\s　​‌‍⁠﻿]+/g` — `\s` + 全角スペース + 6 種の zero-width 文字）
4. 大文字小文字は厳格判定（`toLowerCase` しない、2026-04-29〜）
5. 文末ピリオド「.」「。」の有無は厳格判定（OCR が小さい点を見落とす対策として、確認画面に赤太字の注意書きを表示）
